...
public class UpdateIDViewModel extends BaseViewModel<UpdateIDNavigator, FragmentUpdateIdBinding> {

    private Uri reportImageUri;

    public <V extends ViewDataBinding, N extends BaseNavigator> UpdateIDViewModel(Context mContext, DataManager dataManager, V viewDataBinding, N navigation) {
        super(mContext, dataManager, (UpdateIDNavigator) navigation, (FragmentUpdateIdBinding) viewDataBinding);
    ...
    public void onSaveClicked() {
        if (isValid()) {
            getDataManager().getAccountService().updateID(getMyContext(), true,
                    GeneralFunction.getImageMultipart(reportImageUri.getPath(), "image"), new APICallBack<User>() {
                        @Override

...
public class VerticalSeekBar extends SeekBar {
    ...
    private void initAttrs(AttributeSet attrs) {
        try {
            ...
            indicatorTextOrientation = t.getInt(R.styleable.VerticalRangeSeekBar_rsb_indicator_text_orientation, VerticalRangeSeekBar.TEXT_DIRECTION_VERTICAL);
            t.recycle();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    protected void drawVerticalIndicator(Canvas canvas, Paint paint, String text2Draw) {
        ...
        if (indicatorTextOrientation == VerticalRangeSeekBar.TEXT_DIRECTION_VERTICAL) {
            if (verticalSeekBar.getOrientation() == VerticalRangeSeekBar.DIRECTION_LEFT) {
                degrees = 90;
            } else if (verticalSeekBar.getOrientation() == VerticalRangeSeekBar.DIRECTION_RIGHT) {
                degrees = -90;
            }
        }
        if (degrees != 0) {
